...
const LevelTwoSectionForm: React.FC = () => {
  ...
  useEffect(() => {
    axios.get('http://127.0.0.1:8000/level2-skill-trainings/')
      .then(res => setSkillTrainings(res.data))
      .catch(err => console.error('Error fetching skill trainings:', err));
  }, []);
  ...
  return (
    ...
    //   <h2 className="text-2xl font-bold mb-4">Add Section to Level 2 Training</h2>
    //   <form onSubmit={handleSubmit} className="space-y-4">
    //     <div>
    //       <label className="block font-medium mb-1">Select Skill Training</label>
    //       <select
    ...
    //         required
    //       >
    //         <option value="">-- Select Skill Training --</option>
      ...
      <form onSubmit={handleSubmit} className='space-y-6'>
        <div className='grid grid-cols-1 md:grid-cols-2 gap-6'>
          <div>
            ...
            >
              Select Skill Training <span className='text-red-500'>*</span>
            </label>
            <select
              id='training-select'
              value={selectedTraining ?? ''}
