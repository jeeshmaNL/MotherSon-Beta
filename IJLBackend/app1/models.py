...
class Leveltwodepartments(models.Model): # department like Production
...
class LevelTwoSection(models.Model):
    department = models.ForeignKey(Leveltwodepartments,on_delete=models.CASCADE, related_name='leveltwo_departments')
...
class LevelTwoOJT(models.Model):
    traineeId = models.CharField(max_length=100)
    trainee_name = models.CharField(max_length=100)
    station = models.ForeignKey(LevelTwoTopic, on_delete=models.SET_NULL, null=True, related_name='trainees') #processname
    trainer_name = models.CharField(max_length=100)
    line = models.ForeignKey(LevelTwoSection, on_delete=models.SET_NULL, null=True, related_name='trainees')
    revision_date = models.DateField(null=True, blank=True) #new
    DOJ = models.DateField(null=True, blank=True) #new

    def save(self, *args, **kwargs):
        # Only set DOJ if not already set
        if self.traineeId:
            from .models import OperatorMaster  # import here to avoid circular import
            try:
                operator = OperatorMaster.objects.get(employee_code=self.traineeId)
                self.DOJ = operator.date_of_join
...
class LevelTwoOJTDays(models.Model):
    name = models.CharField(max_length=20)  
    day_number = models.IntegerField(unique=True,blank=True,null=True)  # Add this field
...
class LevelTwoOJTScores(models.Model):
    trainee = models.ForeignKey(LevelTwoOJT, on_delete=models.CASCADE,related_name='ojtscores')
    day = models.ForeignKey(LevelTwoOJTDays, on_delete=models.CASCADE)
    ...
    def save(self, *args, **kwargs):
        # Calculate production marks for this entry
        self.production_marks = self.calculate_production_marks(
            self.production_plan, self.production_actual
        )
        
        super().save(*args, **kwargs)  # Save this score first

        # Now update totals for this trainee across all days
        totals = LevelTwoOJTScores.objects.filter(trainee=self.trainee).aggregate(
            total_production=Sum('production_marks'),
            total_quality=Sum('quality_marks')
        )

        self.total_production_marks = totals['total_production'] or 0
        self.total_quality_marks = totals['total_quality'] or 0

        # Update result based on total production marks (e.g., pass if ≥ 70% of 24)
        if self.total_production_marks >= 0.7 * 24:
            self.result = "Pass"
        else:
            self.result = "Fail"

        super().save(update_fields=['total_production_marks', 'total_quality_marks', 'result'])

    @staticmethod
    def calculate_production_marks(plan, actual):
        plan = float(plan)
        actual = float(actual)
        if plan <= 0:
    ...
    @staticmethod
    def calculate_quality_marks(no_of_rejections):
        no_of_rejections = int(no_of_rejections)
        if no_of_rejections == 0:
            return 4
        elif no_of_rejections <= 2:
            return 2
        elif no_of_rejections <= 5:
            return 1
        else:
            return 0

    def __str__(self):
        return f"{self.trainee.trainee_name} | {self.day.name} | Prod: {self.production_marks} | Qual: {self.quality_marks} | {self.result}"  
...
#  Skill  Matrix IJL
class SkillMatrix(models.Model):
    departments = models.ForeignKey(Leveltwodepartments, on_delete=models.CASCADE, null=True, blank=True)
    line = models.ForeignKey(LevelTwoSection,on_delete=models.CASCADE, related_name='lines',null=True,blank=True)
    last_updated_on = models.DateField()
    next_updation_date = models.DateField()
    doc_no = models.CharField(max_length=50)
    Eff_Date = models.DateField(blank=True, null=True)
    rev_no_date = models.CharField(max_length=10,blank=True, null=True)
    prepared_by = models.CharField(max_length=100, blank=True, null=True)
    approved_by = models.CharField(max_length=100, blank=True, null=True)
    def __str__(self):
        return f"{self.departments} - {self.line}"

# new section model 

class Section(models.Model):
    department=models.ForeignKey(SkillMatrix, on_delete=models.CASCADE, related_name='department',null=True,blank=True)
    name=models.CharField(max_length=100)
...
class MonthPlan(models.Model):
    employee = models.ForeignKey(OperatorMaster,on_delete=models.CASCADE,related_name='employee_name')
    department = models.ForeignKey(Leveltwodepartments,on_delete=models.CASCADE,related_name='department_select')
    line = models.ForeignKey(LevelTwoSection,on_delete=models.CASCADE,related_name='lines_data')
    operation = models.ForeignKey(OperationList,on_delete=models.CASCADE,related_name='operation')
    date = models.DateField()
    plan = models.IntegerField()
    def save(self, *args, **kwargs):
        # Automatically set plan to operation.number
        if self.operation:
            self.plan = self.operation.number
        super().save(*args, **kwargs)
...
class Score(models.Model):
    ...
    def update_skill_matrix(self):
        """Update OperatorLevel in skill matrix when employee passes exam"""
        ...
        for attempt in range(max_retries):
            try:
                ...
                with transaction.atomic():
                    # Find or create corresponding operation in OperationList
                    ...
                    # If exists and new level is higher, update it
                    if not created and level_number > operator_level.level:
                        operator_level.level = level_number
                        operator_level.save()
                        print(f"✅ Updated skill matrix: {self.employee.full_name} - {operation.name} - Level {level_number}")
                    elif created:
...
def update_skill_matrix_on_score_save(sender, instance, created, **kwargs):
    ...
    if instance.passed:
        try:
            ...
            print(f"❌ Signal error: Failed to schedule skill matrix update for score {instance.id}: {str(e)}")
            import traceback
            traceback.print_exc()
    else:
        print(f"ℹ️  Signal: Score {instance.id} did not pass, no skill matrix update needed")
