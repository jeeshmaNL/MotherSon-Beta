...
class LevelTwoOJTSerializer(serializers.ModelSerializer):
    line_name = serializers.CharField(source='line.title', read_only=True)
    station_name = serializers.CharField(source='station.title', read_only=True)
    # ojtscores = LevelTwoOJTScoreSerializer(many=True, read_only=True)
    class Meta:
        model = LevelTwoOJT
        fields = ['traineeId', 'trainee_name', 'station', 'station_name', 'trainer_name', 'line', 'line_name','revision_date','DOJ' ]


    def create(self, validated_data):
        trainee_id = validated_data.get('traineeId')
        if trainee_id:
            from .models import OperatorMaster  # import here to avoid circular import
            try:
                operator = OperatorMaster.objects.get(employee_code=trainee_id)
                validated_data['DOJ'] = operator.date_of_join
            except OperatorMaster.DoesNotExist:
                validated_data['DOJ'] = None
        return super().create(validated_data)

    def update(self, instance, validated_data):
        trainee_id = validated_data.get('traineeId', instance.traineeId)
        if trainee_id:
            from .models import OperatorMaster
            try:
                operator = OperatorMaster.objects.get(employee_code=trainee_id)
...
class LevelTwoOJTDaysSerializer(serializers.ModelSerializer):
    class Meta:
        model = LevelTwoOJTDays
        fields = '__all__'

from rest_framework.response import Response
from rest_framework import status
class LevelTwoOJTScoresSerializer(serializers.ModelSerializer):
    trainee = serializers.PrimaryKeyRelatedField(queryset=LevelTwoOJT.objects.all())
    
    # topic = serializers.PrimaryKeyRelatedField(queryset=LevelTwoTrainingTopic.objects.all())
    day = serializers.PrimaryKeyRelatedField(queryset=LevelTwoOJTDays.objects.all())
    

    class Meta:
        model = LevelTwoOJTScores
        fields = '__all__'
...
from rest_framework import serializers
from .models import LevelTwoOJT, LevelTwoOJTScores

class LevelTwoOJTDayWiseSerializer(serializers.ModelSerializer):
    class Meta:
        model = LevelTwoOJTScores
        exclude = ['id', 'trainee']  # hide trainee since it's repeated in parent


class LevelTwoOJTGroupedSerializer(serializers.ModelSerializer):
    ojtscores = LevelTwoOJTDayWiseSerializer(many=True, read_only=True)
    station_name = serializers.CharField(source='station.title', read_only=True)
    line_name = serializers.CharField(source='line.title', read_only=True)
    ...
    class Meta:
        model = LevelTwoOJT
        fields = [
            'traineeId', 'trainee_name', 'station', 'station_name',
            'trainer_name', 'line', 'line_name', 'revision_date', 'DOJ',
            'ojtscores'
        ]

class NestedLevelTwoOJTScoresSerializer(serializers.ModelSerializer):
    # topic_id = serializers.PrimaryKeyRelatedField(queryset=LevelTwoTrainingTopic.objects.all(), source='topic')
    day_id = serializers.PrimaryKeyRelatedField(queryset=LevelTwoOJTDays.objects.all(), source='day')

    class Meta:
        model = LevelTwoOJTScores
        fields = ['id', 'day_id', 'date', 'production_plan', 'production_actual', 'production_marks', 'quality_no_of_rejection', 'quality_marks', 'Engineer_judge', 'prepared_by', 'approved_by']


class NestedLevelTwoOJTSerializer(serializers.ModelSerializer):
    ojtscores = NestedLevelTwoOJTScoresSerializer(many=True, read_only=True)
    
    station = serializers.PrimaryKeyRelatedField(queryset=LevelTwoTopic.objects.all())
    line = serializers.PrimaryKeyRelatedField(queryset=LevelTwoSection.objects.all())
    ...
    def create(self, validated_data):
        ...
        trainee = LevelTwoOJT.objects.create(**validated_data)
        if scores_data:
            for score_data in scores_data:
                LevelTwoOJTScores.objects.create(trainee=trainee, **score_data)
        return trainee
    ...
    def update(self, instance, validated_data):
        ...
        if scores_data:
            LevelTwoOJTScores.objects.filter(trainee=instance).delete()
            for score_data in scores_data:
                LevelTwoOJTScores.objects.create(trainee=instance, **score_data)
...
class SkillMatrixSerializer(serializers.ModelSerializer):
    class Meta:
        model = SkillMatrix
        fields = [
            'id','departments','line','last_updated_on','next_updation_date','doc_no','Eff_Date','rev_no_date','prepared_by','approved_by'
        ]
...
class OperationListSerializer(serializers.ModelSerializer):
    ...
    line =serializers.CharField(source='matrix.line',read_only=True)
    class Meta:
        model = OperationList
        fields = ['id','matrix','department','line','number','name','minimum_skill_required']



from rest_framework import serializers
from .models import MonthPlan, OperatorMaster, Leveltwodepartments, LevelTwoSection, OperationList
...
class MonthPlanSerializer(serializers.ModelSerializer):
    ...
    department = serializers.PrimaryKeyRelatedField(
        queryset=Leveltwodepartments.objects.all(),
        write_only=True
    )
