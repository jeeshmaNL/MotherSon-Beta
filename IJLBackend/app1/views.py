...
from .serializers import (
    LevelTwoOJTSerializer,
    # LevelTwoTrainingTopicSerializer,
    LevelTwoOJTDaysSerializer,
    LevelTwoOJTScoresSerializer,
    LevelTwoOJTGroupedSerializer,
)

class LevelTwoOJTViewSet(viewsets.ModelViewSet):
    queryset = LevelTwoOJT.objects.all()
    serializer_class = LevelTwoOJTSerializer
...
class LevelTwoOJTDaysViewSet(viewsets.ModelViewSet):
    queryset = LevelTwoOJTDays.objects.all()
    serializer_class = LevelTwoOJTDaysSerializer


class LevelTwoOJTScoresViewSet(viewsets.ModelViewSet):
    queryset = LevelTwoOJTScores.objects.all()
    serializer_class = LevelTwoOJTScoresSerializer

from rest_framework import generics
from .models import LevelTwoOJT

class LevelTwoOJTGroupedListView(generics.ListAPIView):
    queryset = LevelTwoOJT.objects.prefetch_related('ojtscores').all()
    serializer_class = LevelTwoOJTGroupedSerializer

from rest_framework import viewsets
from .models import LevelTwoOJT
from .serializers import NestedLevelTwoOJTSerializer

class NestedLevelTwoOJTViewSet(viewsets.ModelViewSet):
    queryset = LevelTwoOJT.objects.all()
    serializer_class = NestedLevelTwoOJTSerializer
...
class GetTraineeByCodeView(APIView):
    def get(self, request, trainee_id, station_id):
        try:
            ...
            trainee = LevelTwoOJT.objects.filter(traineeId=trainee_id, station_id=station_id).first()
            if not trainee:
                return Response({"error": "Trainee not found with this ID and station"}, status=status.HTTP_404_NOT_FOUND)
        except Exception as e:
            return Response({"error": f"Error fetching trainee: {str(e)}"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)

        # ✅ Call function to calculate and save status
        # trainee.calculate_and_save_training_status()  # Commented out - method doesn't exist

        serializer = NestedLevelTwoOJTSerializer(trainee)
        return Response(serializer.data, status=status.HTTP_200_OK)    
...
from .serializers import MonthPlanSerializer

class MonthPlanViewSet(viewsets.ModelViewSet):
    queryset = MonthPlan.objects.all()
...
class MultiSkillingViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        ...
        MonthlySkill.objects.create(multiskilling=instance)
...
class AllEmployeesWithActiveSkillsView(APIView):
    def get(self, request):
        ...
        # Filter operators by full_name (case-insensitive)
        operators = OperatorMaster.objects.filter(full_name__icontains=name_query)

        result = []

        for operator in operators:
            # Fetch only active skills for this operator
            ...
            skills = [
                {
                    "skill_level": skill.skill_level,  # skill_level is CharField according to your model
                    "date": skill.date,
                    "notes": skill.notes,
                    "status": skill.status,
                    # If you want department, section, operation names, you can add them here:
                    "department": skill.department.name if skill.department else None,
                    "section": skill.section.name if skill.section else None,
                    "operation": skill.operation.name if skill.operation else None,
                }
                for skill in active_skills
